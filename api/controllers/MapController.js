...
// 		I want the biggest cities to be about ⅛ the current size, which would make London about 1,000,000 while making the smaller cities (around 1,000,000) about ¼ the current size, and cities around 100,000 about ½ the current size.
...
// 		Y = targetGamePop, X = currentRealPop
// 		Y = 10,000(5^log_10(X/10000))
...
//Every city has 3 "rings" the inner, middle and outer rings. These are the areas accessible by short, medium and long range stations.
//This function will determine how the population will be spread out using the citySprawl score.
						...
						//3 is the easiest (and most dense), you just fill everything in.
						var ringUnits = {};
						 ringUnits.inner = 0;
						 ringUnits.middle = 0;
						 ringUnits.outer = 0;
					...
					//citySprawl score of 1 means that there is one unit placed in each available ring.
								if(citySprawl == 1){
									ringUnits.inner++;
									totalCityUnits--;
									if(totalCityUnits==0){return ringUnits}
					...
					//citySprawl score of 2 means that two units will be placed in the smallest available ring, then one
					//in the next available ring. Then start over.
								if(citySprawl == 2){
									ringUnits.inner++;
									totalCityUnits--;
									if(totalCityUnits==0){return ringUnits}
					...
					//citySprawl score of 3 means that all units will be placed as densely as possible in the smallest available rings.
								if(citySprawl == 3){
									ringUnits.inner++;
									totalCityUnits--;
									if(totalCityUnits==0){return ringUnits}
